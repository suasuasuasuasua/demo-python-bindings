...
TEST(Derivative, Sin) {
    ...
    EXPECT_NEAR(myMathLib::calculus::derivative([](double x) { return std::sin(x); }, 0.0), 1.0,
                1e-4);
}

TEST(Derivative, NegativeStepThrows) {
    EXPECT_THROW(myMathLib::calculus::derivative([](double x) { return x; }, 1.0, -1.0),
...
TEST(Integrate, Quadratic) {
    ...
    EXPECT_NEAR(myMathLib::calculus::integrate([](double x) { return x * x; }, 0.0, 1.0, 10000),
                1.0 / 3.0, 1e-4);
}
...
TEST(Integrate, SinOverPi) {
    ...
    EXPECT_NEAR(
        myMathLib::calculus::integrate([](double x) { return std::sin(x); }, 0.0, pi, 10000), 2.0,
        1e-4);
}

TEST(Integrate, ZeroIntervalsThrows) {
    EXPECT_THROW(myMathLib::calculus::integrate([](double x) { return x; }, 0.0, 1.0, 0),
                 std::invalid_argument);
}

TEST(ConvergingIntegrate, Constant) {
    // integral of 1 from 0 to 5 = 5
    EXPECT_NEAR(myMathLib::calculus::converging_integrate([](double) { return 1.0; }, 0.0, 5.0),
                5.0, 1e-4);
}

TEST(ConvergingIntegrate, Linear) {
    // integral of x from 0 to 1 = 0.5
    EXPECT_NEAR(myMathLib::calculus::converging_integrate([](double x) { return x; }, 0.0, 1.0),
                0.5, 1e-4);
}

TEST(ConvergingIntegrate, EqualBoundsIsZero) {
    EXPECT_NEAR(myMathLib::calculus::converging_integrate([](double x) { return x; }, 2.0, 2.0),
                0.0, 1e-9);
}
